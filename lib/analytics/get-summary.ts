...
function getDateRange(days: PeriodDays): { start: string; end: Date } {
  const end = new Date();
...
export interface AnalyticsSummaryResponse {
  ...
  comparison?: {
    completedTasks: PeriodComparison;
    newContacts: PeriodComparison;
    newDeals: PeriodComparison;
    contentPosts: PeriodComparison;
...
export async function getAnalyticsSummary(
  ...
  const { start: startDate, end: now } = getDateRange(days);
  ...
  const prevStartDate = new Date(new Date(startDate));
  ...
  // --- Period comparison ---
  if (compare) {
    // Current period counts (from already-fetched data)
    const currentCompletedTasks = tasksInPeriod.length;
    const currentContentPosts = contentData.length;

    // Current period: contacts and deals created in period
    const [
      currentNewContacts,
      currentNewDeals,
      prevCompletedTasks,
    ...
    // Net income for current and previous periods
    ...
    try {
      ...
      for (const tx of currentTx.data ?? []) {
        const amt = Math.abs(Number(tx.amount) || 0);
        currentNetIncome += tx.type === "income" ? amt : -amt;
      }
      for (const tx of prevTx.data ?? []) {
        const amt = Math.abs(Number(tx.amount) || 0);
        prevNetIncome += tx.type === "income" ? amt : -amt;
      }
    } catch {
      // keep defaults
    }
    ...
    response.comparison = {
      ...
      newContacts: {
        current: currentNewContacts,
        previous: prevNewContacts,
      },
      newDeals: {
        current: currentNewDeals,
        previous: prevNewDeals,
      },
      contentPosts: {
        current: currentContentPosts,
        previous: prevContentPosts,
      },
      netIncome: {
        current: Math.round(currentNetIncome * 100) / 100,
        previous: Math.round(prevNetIncome * 100) / 100,
      },
    };
  }
